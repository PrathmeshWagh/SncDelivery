...
const Delivered: FC<Props> = (props): JSX.Element => {
  ...
  const Delivered = async () => {
    ...
    try {
      setLoading(true);
      const api: any = await getMethod(`deliveryman/get-delivered-order-list?date=${apiDate}`);
      if (api.status === 200) {
            ...
            <Pressable key={index} onPress={() =>  navigation.dispatch(
              ...
              <View style={styles.detailedView}>
                ...
                <Text style={styles.redText}>{translations.order_listing.Payment_Method}: </Text>
                  <Text style={styles.detailedText}>{item.new_payment_type}{''}({item.final_amount})</Text>
                </View>
              </View>
            </Pressable>
          ))
